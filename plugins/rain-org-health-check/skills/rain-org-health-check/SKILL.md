...
description: >-
  ...
  soldeer publish gaps, and untested external/public functions on concrete
  Solidity contracts. Also reports when each repo was last fully (whole-repo)
...
| `soldeer-unpublished`                        | the repo names a soldeer package (release-metadata table, or `package-release.yaml`'s `soldeer-package:` input after rainix#335) but no revision is on the registry                                                                                                                                                                                                                                             | a publishable package never got pushed — wire `rainix-autopublish` (+ `[package].version`), add a `.soldeerignore` (publish only `src/` + license/readme; soldeer's sensitive-file prompt otherwise hangs CI), and have an org admin create the project on soldeer.xyz before the first push.                                                     |
