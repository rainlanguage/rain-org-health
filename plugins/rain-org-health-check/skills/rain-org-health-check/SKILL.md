...
| `manual-release-not-autopublish` | repo still has a `manual-release.yaml` (manual `workflow_dispatch` release) and no autopublish/`package-release` workflow — the org standard is merge-driven autopublish, not a manual trigger | replace `manual-release.yaml` with a `package-release.yaml` calling the shared `rainix-autopublish` reusable (`on: push: branches: [main]`, `secrets: inherit`), passing the relevant inputs — `crates: <cargo crate names>`, `npm-package: <@scope/pkg>`, and/or `soldeer-package: <name>`; the reusable bumps + publishes crates / npm / soldeer in lockstep on merge. Worked example: dotrain#141. |
