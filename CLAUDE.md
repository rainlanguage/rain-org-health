...
- `site/` — the dashboard: one self-contained page per view (inline CSS/JS)
  - `health.json`. `repositories.html` is the landing page; `index.html` is a
    redirect to it, kept because Pages serves it as the site root. Deployed by
    `pages.yml`. Nothing is fetched from a CDN or any third-party host at
    runtime: a library the pages genuinely need (the ELK layout engine) is
    vendored into `site/vendor/` (byte-identical to its published release,
    prettier-ignored) and loaded from the same origin. Pan and zoom are not such
    a need — the browser scrolls and pinch-zooms natively, and its zoom focuses
