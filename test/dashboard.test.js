...
// The dashboard renders cross-repo, attacker-influenceable strings (issue titles,
// producer reasons, run outcomes). Interpolating any of them into `innerHTML` is an
// HTML-injection sink, and a hand-rolled escaper applied per site is
// escape-by-remembering — one forgotten sink is a hole. The guarantee is structural:
// `innerHTML` may only ever be assigned a CONSTANT (clearing it, or fixed
// empty-state markup), never a template carrying `${}`. Data goes through
// createElement/textContent instead, which escapes by construction. Pinned at the
// source level so a new renderer cannot quietly reintroduce the class.
Deno.test("site: innerHTML is never assigned interpolated data", () => {
  const offenders = [];
  for (const f of Deno.readDirSync(new URL("../site", import.meta.url))) {
    if (!f.name.endsWith(".html")) continue;
    const src = Deno.readTextFileSync(
    ...
    // Each `.innerHTML =` assignment, up to the statement's end.
    for (const m of src.matchAll(/\.innerHTML\s*=\s*([\s\S]*?);\n/g)) {
      const rhs = m[1];
      // A backtick template carrying `${` is interpolation; so is any assignment
      // built by concatenating a non-literal (an identifier holding built markup).
      if (/\$\{/.test(rhs) || /\+\s*[A-Za-z_$]/.test(rhs)) {
        offenders.push(`${f.name}: ${rhs.replace(/\s+/g, " ").slice(0, 70)}`);
      }
    }
  }
  assert(
    offenders.length === 0,
    "innerHTML must only ever be assigned a constant — build data with " +
      "createElement/textContent instead: " + offenders.join(" | "),
  );
});
...
// helper unnoticed.
// The chart builds DOM nodes rather than an innerHTML string (data must render
// inert by construction), so `wrap.innerHTML` is never assigned. Serialize the
// tree the render actually produced, so these tests keep asserting on rendered
// output — markup, attributes and text — exactly as before.
function serializeNode(n) {
  if (n == null) return "";
  if (typeof n !== "object") return String(n);
  // svgEl carries `class` in attrs (setAttribute); htmlEl carries it in className.
  const cls = n.className ? ` class="${n.className}"` : "";
  const attrs = Object.entries(n.attrs || {})
    .map(([k, v]) => ` ${k}="${v}"`)
    .join("");
  const inner = n._text !== undefined
    ? String(n._text)
...
function pmChart(runs, pmMode = "pct") {
  ...
  const svgEl = bind("metrics.html", "svgEl", ["document", "SVGNS"], [
    document,
    SVGNS,
  ]);
  const htmlEl = bind("metrics.html", "htmlEl", ["document"], [document]);
  ...
  return wrap.children.map(serializeNode).join("");
}
